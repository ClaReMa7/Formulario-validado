const submitFunction = (event) => {
    if (!validarFormulario()){
        event.preventDefault(); //que se prevenga la actualización de la web
    }else{
...
function validarFormulario() {
    ...
    if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(email.value)) { // este regex (expresión regular) valida que el formato del email sea válido
...
//limpiart todos los campos de entrada
const limpiarFormulario = () => {
    document.querySelectorAll('input[type="text"], input[type="email"], input[type="number"], select').
